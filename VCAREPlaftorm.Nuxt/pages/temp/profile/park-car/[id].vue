...
const { modifyUserCar, getCarGroupWithModels, getUsersCarById } = useGarage();

// States
const loading = ref(false);
const errMessage = ref(null);
const tab = ref(0);
const tabs = ['ویرایش خودرو'];
...
// Logics
// OnMount first get the car brands and car models
// then fetch user car and based on car models and brands fill initial data
...
getCarGroupWithModels().then(async (r) => {
  if (r.status === 200) {
    ...
    //get user car info and if 200 fill the initial object
    getUsersCarById(route.params.id).then(async (r) => {
      if (r.status == 200) {
        Object.assign(initialValues, r.data);
        loading.value = true;
        errMessage.value = null;
        if (carBrandas.value !== null) {
          selectModel(r.data.carGroupId)
        }
        loading.value = false;
...
const selectModel = (id) => {
  let category = carGroupsWithModels.find((item) => item.id == id);
  carModels.value = category.models
};

const goPrevPage = ()=>{
  router.push({path:'/profile/my-cars'});
}
  ...
  <div class="flex flex-col gap-2 w-full farsi-num">
    ...
    <div v-if="tab === 0" class="flex-1 flex flex-col gap-7 items-center border rounded-md px-2 md:px-16 py-10">
      ...
      <VForm :validation-schema="schema" :initial-values="initialValues" @submit="handleFormSubmit"
        ...
        <ElementsVSelectInput v-if="carModels" name="carModelId" label="مدل خودرو" :options="carModels"
          :requiredStar="true" placeholder="انتخاب کنید" />
        ...
        <div
          ...
          <button @click="goPrevPage()" type="reset"
