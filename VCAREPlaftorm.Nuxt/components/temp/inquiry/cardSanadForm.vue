...
const { inquiryCardSanad, sendInquiryOTP } = useInquiry();
...
const sanadStatus = ref();
const fakeData = {
  cardPrintDate: "1401/08/29",
  cardPostalBarcode: "18540000017068842828",
  cardStatusTitle: "تحويل شده به پست",
  documentPrintDate: "1401/09/08",
  documentStatusTitle: "صدور",
  plateWord: "ایران ۵۰ - ۸۶۷ ج  ۷۶",
};
      ...
      .then((r) => {
        if (r.status == 200) {
          ...
          window.dataLayer.push({
            ...
            source: 'card-sanad-desktop'
          })
          sanadStatus.value = r.data;
...
<template>
  <div>
    ...
    <SharedCarsModal
      v-if="carsModal"
      @close="carsModal = false"
      :cars="cars.data"
      @selectCarID="selectCar"
    />
    <div v-if="sanadStatus" class="flex flex-col space-y-3">
      <button
        @click="newInquiry"
      ...
      <div class="flex flex-col space-y-3 max-h-96 bg-white overflow-y-auto">
        <div class="flex flex-col space-y-4">
          <div
            class="flex flex-col border border-blue-500 rounded-md overflow-x-auto"
          >
            <div
            ...
            <div class="flex flex-col">
              <div class="flex justify-between bg-blue-100 py-2 px-3">
                <p>وضعیت کارت</p>
                <p>
                  {{ sanadStatus.cardStatusTitle }}
                </p>
              </div>
              <div class="flex justify-between py-2 px-3">
                <p>تاریخ چاپ کارت</p>
                <p>{{ sanadStatus.cardPrintDate }}</p>
              </div>
              <div class="flex justify-between bg-blue-100 py-2 px-3">
                <p>بارکد پستی کارت</p>
                <p>{{ sanadStatus.cardPostalBarcode }}</p>
              </div>
              <div class="flex justify-between py-2 px-3">
                <p>وضعیت سند</p>
                <p>{{ sanadStatus.documentStatusTitle }}</p>
              </div>
              <div class="flex justify-between bg-blue-100 py-2 px-3">
                <p>تاریخ چاپ سند</p>
                <p>{{ sanadStatus.documentPrintDate }}</p>
              </div>
              <div class="flex justify-between py-2 px-3">
                <p>شماره پلاک</p>
                <p>{{ sanadStatus.plateWord }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div>
        ...
        >
          <div class="flex shrink-0 flex-col text-right">
            ...
            <VErrorMessage
              name="FirstTwoDigits"
              class="text-xs font-normal text-red-500 mt-1 farsi-num"
            />
          </div>
